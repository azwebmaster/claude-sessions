...
`AgentToolDiagram` is an interactive radial SVG: the root agent sits at the center, subagents on an inner ring, and tools on one or more expanded outer rings near the agents that call them. Agent circles can be sized by **peak context** or **total tokens** via a toolbar toggle (default: peak context); tool circles use attributed context growth. Curved arrows show call flow (stroke weight = call volume). Support pan, wheel/+/− zoom, and drag-to-rearrange; include an **Arrange** control that re-runs the link-aware radial auto-layout (with overlap separation) and fit-to-view, plus a separate fit-view control. Use `nodeKindStyle` for node accents and `focusHighlight` when an agent or tool is selected. Default to the top N tools by volume so dense sessions stay readable; offer an **All tools / Expand diagram** control that reveals every tool, grows the viewport, and adds concentric tool rings as needed. Note hidden tool count in the caption when collapsed.
